...
exports.solicitaTel = async (req, res)=>{
    ...
    const requestExist = await RequestTel.find({userRequest: userLogged, userResponse: id})
...
exports.findRequest = async (req, res) =>{
    ...
    for(let i = 0;  i < requestList.length; i++){
        ...
        if(requestList[i].userRequest == userLogged){
            ...
            var userResponse = await User.find({_id: requestList[i].userResponse});
        ...
        if(requestList[i].userResponse == userLogged){
            ...
            var userRequest = await User.find({_id: requestList[i].userRequest});
